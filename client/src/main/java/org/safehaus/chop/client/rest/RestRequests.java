...
import static org.safehaus.chop.api.ConfigKeys.PARAM_RUNNER_WAR;
...
public class RestRequests {
     ...
     * @param runnerWithBucket the runner war path with bucket prefix query parameter value
     * @param propagate whether or not to enable propagation
     *
     * @return the result of the operation
     */
    public static Result load( Runner runner, String runnerWithBucket, Boolean propagate ) {
        DefaultClientConfig clientConfig = new DefaultClientConfig();
        Client client = Client.create( clientConfig );
        WebResource resource = client.resource( runner.getUrl() ).path( "/load" );
        return resource.queryParam( PARAM_PROPAGATE, propagate.toString() ).queryParam( PARAM_RUNNER_WAR, runnerWithBucket )
